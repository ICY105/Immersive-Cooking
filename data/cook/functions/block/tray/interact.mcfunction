...
execute if score $in_0 cook_data matches 0 if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,predicate=!cook:holding_ingrediant,distance=..8,nbt=!{SelectedItem:{ tag:{plate:1b} }}] run function cook:block/tray/interact_destroy_holder

...
execute if score $in_0 cook_data matches 0 if score @s cook_data matches 60.. if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,distance=..8,nbt={SelectedItem:{ tag:{plate:1b} }}] unless data entity @p[tag=cook_interact] Inventory[35] run function cook:block/frying_pan/fill_plate
execute if score $in_0 cook_data matches 0 if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,predicate=!cook:holding_ingrediant,distance=..8] unless data entity @p[tag=cook_interact] Inventory[35] run function cook:block/frying_pan/interact_destroy_holder

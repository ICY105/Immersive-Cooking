...
execute if score $in_0 cook_data matches 0 if entity @s[tag=cook_has_item] if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,predicate=!cook:holding_ingrediant,distance=..8] run function cook:block/smoking_rack/destroy_item_holder

...
execute if score $in_0 cook_data matches 0 if score @s cook_data matches 60.. if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,distance=..8,nbt={SelectedItem:{id:"minecraft:bowl"}}] run function cook:block/cooking_pot/fill_bowl
execute if score $in_0 cook_data matches 0 if entity @p[tag=cook_interact,predicate=du:entity/is_sneaking,predicate=!cook:holding_ingrediant,distance=..8] run function cook:block/cooking_pot/interact_destroy_holder
